...
def RAantenna_inplace(RRs, IIs, cosi, psi, phi, costh, ts, FFs, nf_low, NTs, kdotx):
    ...
    for itrc in range(0, 3):
        betas[itrc] = 2/3*np.pi*itrc+wc.lambda0
        sbs[itrc] = (wc.AU/wc.Larm*wc.ec)*np.sin(betas[itrc])
        cbs[itrc] = (wc.AU/wc.Larm*wc.ec)*np.cos(betas[itrc])
    ...
    for n in range(0, NTs):
        # TODO should be able to eliminate xas
        # Barycenter time
        # t = TF[n]
        # pull out Larm scaling

        alpha = 2*np.pi*wc.fm*ts[n+nf_low]+wc.kappa0
        sa = np.sin(alpha)
        ca = np.cos(alpha)
        for itrc in range(0, 3):
            x[itrc] = (wc.AU/wc.Larm)*ca + sa*ca*sbs[itrc] - (1. + sa*sa)*cbs[itrc]
            y[itrc] = (wc.AU/wc.Larm)*sa + sa*ca*cbs[itrc] - (1. + ca*ca)*sbs[itrc]
            z[itrc] = -wc.SQ3*(ca*cbs[itrc] + sa*sbs[itrc])
        ...
        ya = (y[0]+y[1]+y[2])/3
        za = (z[0]+z[1]+z[2])/3
        kdotx[n] = (wc.Larm/wc.CLIGHT)*(xa*kv[0]+ya*kv[1]+za*kv[2])
        # xa = xas[n]
        # ya = yas[n]
        # za = zas[n]

        fr = 1/(2*wc.fstr)*FFs[n+nf_low]
        ...
        # These are not unit vectors. Just pulling out the wc.Larm scaling
        r10[0] = xa-x[0]
        r10[1] = ya-y[0]
        r10[2] = za-z[0]
        r20[0] = xa-x[1]
...
#        p1 = -fr2*np.sin(fr2)
        ...
        for i in range(0, 3):
            fprs[i] = -((dplus[i, (i+1) % 3]*cosps+dcross[i, (i+1) % 3]*sinps)*TR[i, (i+1) % 3]
                        - (dplus[i, (i+2) % 3]*cosps+dcross[i, (i+2) % 3]*sinps)*TR[i, (i+2) % 3])/2
            fcrs[i] = -((-dplus[i, (i+1) % 3]*sinps+dcross[i, (i+1) % 3]*cosps)*TR[i, (i+1) % 3]
                        - (-dplus[i, (i+2) % 3]*sinps+dcross[i, (i+2) % 3]*cosps)*TR[i, (i+2) % 3])/2
            fpis[i] = -((dplus[i, (i+1) % 3]*cosps+dcross[i, (i+1) % 3]*sinps)*TI[i, (i+1) % 3]
                        - (dplus[i, (i+2) % 3]*cosps+dcross[i, (i+2) % 3]*sinps)*TI[i, (i+2) % 3])/2
            fcis[i] = -((-dplus[i, (i+1) % 3]*sinps+dcross[i, (i+1) % 3]*cosps)*TI[i, (i+1) % 3]
                        - (-dplus[i, (i+2) % 3]*sinps+dcross[i, (i+2) % 3]*cosps)*TI[i, (i+2) % 3])/2

        FpRs[0] = (2*fprs[0]-fprs[1]-fprs[2])/3.*Aplus
        FcRs[0] = (2*fcrs[0]-fcrs[1]-fcrs[2])/3.*Across

        FpRs[1] = (fprs[2]-fprs[1])/wc.SQ3*Aplus
        FcRs[1] = (fcrs[2]-fcrs[1])/wc.SQ3*Across
        ...
        FpIs[1] = (fpis[2]-fpis[1])/wc.SQ3*Aplus
        FcIs[1] = (fcis[2]-fcis[1])/wc.SQ3*Across
...
def get_xis_inplace(kv, ts, xas, yas, zas, xis):
    ...
    kdotx = (xas*kv[0]+yas*kv[1]+zas*kv[2])*wc.Larm/wc.CLIGHT
    xis[:] = ts-kdotx
...
def spacecraft_vec(ts):
    ...
    alpha = 2*np.pi*wc.fm*ts+wc.kappa0

    sa = np.sin(alpha)
    ca = np.cos(alpha)

    for i in range(0, 3):
        beta = i*2/3*np.pi+wc.lambda0
        sb = np.sin(beta)
        cb = np.cos(beta)
        xs[i] = wc.AU/wc.Larm*ca + wc.AU/wc.Larm*wc.ec*(sa*ca*sb - (1. + sa*sa)*cb)
        ys[i] = wc.AU/wc.Larm*sa + wc.AU/wc.Larm*wc.ec*(sa*ca*cb - (1. + ca*ca)*sb)
        zs[i] = -wc.SQ3*wc.AU/wc.Larm*wc.ec*(ca*cb + sa*sb)
    # guiding center
